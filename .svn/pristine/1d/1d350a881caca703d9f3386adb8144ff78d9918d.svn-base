...
//        pageViews = []
        ...
        if pageViews.count != dataSource.numberOfPages(in: self) {
            pageViews = Array.init(repeating: UIView(), count: dataSource.numberOfPages(in: self))
        }

        self.contentSize = CGSize(width: frame.width * CGFloat(dataSource.numberOfPages(in: self)), height: frame.height)
        
        self.prepareMorePages(currentIndex: self.currentIndex)

//        let end = (currentIndex + 1) < dataSource.numberOfPages(in: self) ? (currentIndex + 1) : currentIndex
//        for i in 0...end {
...
//                    make.left.equalTo(CGFloat(i) * SWScreen_width)
...
//        }
    ...
    public func prepareMorePages(currentIndex: Int) {
        guard let dataSource = dataSource, dataSource.numberOfPages(in: self) > 0 else { return }
        guard currentIndex < dataSource.numberOfPages(in: self) else { return }
        if pageViews.count != dataSource.numberOfPages(in: self) {
            pageViews = Array.init(repeating: UIView(), count: dataSource.numberOfPages(in: self))
