...
+ (PSLMDBKeyValueStore *)uploadedMediaStore
{
    ...
    {
        NSString *documentsPath = [DQDTelegraphManager dqd_documentsPath];
                ...
                if (strongSelf != nil) {
                    if ([result isKindOfClass:[TLDocument$document class]]) {
                        ...
                        if (localDocumentMessage.localThumbnailDataPath != nil)
                        {
                            ...
                            if (image != nil)
                            {
                                NSData *thumbnailData = UIImageJPEGRepresentation(TGScaleImageToPixelSize(image, TGFitSizeNew(image.size, CGSizeMake(90, 90))), 0.6f);
...
- (void)_uploadDownloadedData:(NSData *)data dispatchThumbnail:(bool)dispatchThumbnail
{
    ...
    else if ([self.preparedMessage isKindOfClass:[TGPreparedDownloadDocumentMessage class]])
    {
        ...
        if (image != nil)
        {
            image = TGScaleImageToPixelSize(image, TGFitSizeNew(image.size, CGSizeMake(90, 90)));
            if (image != nil)
