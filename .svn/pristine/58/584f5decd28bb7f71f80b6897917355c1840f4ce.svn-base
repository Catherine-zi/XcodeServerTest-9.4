...
- (void)reloadData
{
    ...
    if (_stickerPack == nil)
    {
        ...
        [_recentStickersDisposable setDisposable:[[[TGBridgeStickersSignals recentStickersWithLimit:16] deliverOn:[SQueue mainQueue]] startWithNext:^(NSArray *recent)
        {
            __strong TGStickersController *strongSelf = weakSelf;
            if (strongSelf == nil)
                return;
...
{
    return [TGStickersHeaderController class];
}

- (void)table:(WKInterfaceTable *)table updateHeaderController:(TGStickersHeaderController *)controller
{
    [controller update];
}
...
{
    return [TGStickersSectionHeaderController class];
}

- (void)table:(WKInterfaceTable *)table updateSectionController:(TGStickersSectionHeaderController *)controller forSection:(NSUInteger)section
{
    if (section == 0)
    {
        if (_stickerPack == nil)
