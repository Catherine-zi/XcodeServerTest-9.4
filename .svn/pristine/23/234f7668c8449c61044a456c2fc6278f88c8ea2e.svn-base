...
//#import "RMIntroViewController.h"  //Avazu 20180608 ？？？
...
//#import <HockeySDK/HockeySDK.h>   //Avazu 20180608 ？？？
...
@interface TGAppDelegate () <AVAudioPlayerDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate>
{
    bool _inBackground;
...
{
	[MTA startWithAppkey:@"IPV8ES5HN53Y"];
	[[SPUserEventsManager sharedManager] addCountForEvent:SWUEC_ShowWelcomeImage];

    [Bugly startWithAppId:@"73e3606a49"];
//	[NetworkListener monitorNetWithBlock:^(BOOL isReachable) {
//
//		if (isReachable) {
//			[self listenAndaddProxy];
//		}
//	}];
    ...
    [JPUSHService setupWithOption:launchOptions appKey:@"1101bcbc7fcb4b8dfc4f90ce"
                          channel:@"ios-default"
	...
	SWTabBarController *rootTab = [[SWTabBarController alloc]init];
	self.tabBarController = rootTab;
	self.window.rootViewController = rootTab;//_rootController;
    PGTock;
...
{
    application.applicationIconBadgeNumber += SwiftNotificationManager.shared.notificationCount;
...
- (void)applicationDidBecomeActive:(UIApplication *)__unused application
{
    ...
    //[ActionStageInstance() reqqquestActor:@"/tg/locationServicesState/(dispatch)" options:[[NSDictionary alloc] initWithObjectsAndKeys:[[NSNumber alloc] initWithBool:true], @"dispatch", nil] watcher:TGTelegraphInstance];
...
//qq--add 新增校验是否需要添加 代理
- (void)listenAndaddProxy{
//	NSDate *date = [NSDate date];
	NSURL *url = [NSURL URLWithString:@"https://www.google.com/generate_204"];
	NSMutableURLRequest *gRequest = [NSMutableURLRequest requestWithURL:url];
	gRequest.timeoutInterval = 5;
	[[[NSURLSession sharedSession] dataTaskWithRequest:gRequest completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {

		if (data) {

//			[TGDatabaseInstance() setCustomProperty:@"socksProxyData" value:[NSData data]];

			[[[TGTelegramNetworking instance] context] updateApiEnvironment:^MTApiEnvironment *(MTApiEnvironment *apiEnvironment) {
				return [apiEnvironment withUpdatedSocksProxySettings:nil];
			}];
		}else{
			//config proxy
			bool inactive = false;
			MTSocksProxySettings *updatedSettings = [[MTSocksProxySettings alloc] initWithIp:@"94.130.59.227" port:10808 username:@"avazudev" password:@"70HqTONm44nD"];
			...
			NSData *svaData = [NSKeyedArchiver archivedDataWithRootObject:dict];
			
			[TGDatabaseInstance() setCustomProperty:@"socksProxyData" value:svaData];
...
//		NSLog(@"luanchtimer = %f",[newDate timeIntervalSinceDate:date]);

	}] resume] ;
}
//qq--add
- (void)changeChatsTabBar: (BOOL)islogin {

    if ([self.tabBarController isKindOfClass:[SWTabBarController class]]) {
        SWTabBarController *tempTab = (SWTabBarController *)self.tabBarController;
		NSMutableArray *tabBarVcs = [NSMutableArray arrayWithArray:self.tabBarController.viewControllers];
		UIViewController *tgRoot = [tempTab resetChatsTab:islogin];
		tabBarVcs[1] = tgRoot;
		self.tabBarController.viewControllers = tabBarVcs.copy;
    }
}
//qq--add
- (void)addTelegramUserInfo {
	[self getUserInfo];
}
- (void)getUserInfo {
	//用户登录成功
	...
	TelegramUserInfo.shareInstance.firstName = user.firstName.length > 0 ? user.firstName : @"";
	NSLog(@"%@",TelegramUserInfo.shareInstance.firstName);
	TelegramUserInfo.shareInstance.lastName = user.lastName.length > 0 ? user.lastName : @"";
	TelegramUserInfo.shareInstance.userName = user.userName.length > 0 ? user.userName : [NSString stringWithFormat:@"%@ %@",TelegramUserInfo.shareInstance.firstName,TelegramUserInfo.shareInstance.lastName];
	TelegramUserInfo.shareInstance.phoneNumber = user.phoneNumber.length > 0 ? user.phoneNumber : @"";
	TelegramUserInfo.shareInstance.telegramLoginState = @"yes";
	NSString *userId = [NSString stringWithFormat:@"%d",user.uid];
	if (userId.length > 0) {
		[TelegramUserInfo.shareInstance requestNetworkLoginWithUserID:userId];
	}
	...
	[TelegramUserInfo.shareInstance getFirstRecUrlWithBlock:^(NSString * link) {
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			//通过search 获取对应的ID
			[TGAppDelegateInstance.rootController.dialogListController addSearchContext:link withBlock:^(id item) {
				...
				if ([item isKindOfClass:[TGConversation class]])
				{
					...
					dispatch_async(dispatch_get_main_queue(), ^{
						...
						//调用原有的代码进行join
						[[[TGChannelManagementSignals joinTemporaryChannel:conversation.conversationId] deliverOn:[SQueue mainQueue]] startWithNext:nil error:^(__unused id error) {
...
//qq--add,这里是登录成功后原先的跳转到mainController逻辑
- (void)presentMainController
{
	...
	//qq--
	[self changeChatsTabBar:YES];
	[self addTelegramUserInfo];
	[[NSNotificationCenter defaultCenter] postNotificationName:@"SWuserLogin" object:nil];
...
- (void)application:(UIApplication *)application didRegisterUserNotificationSettings:(UIUserNotificationSettings *)__unused notificationSettings
{
    ...
    [SwiftNotificationManager.shared checkNotificationRegistration];
}
...
- (void)application:(UIApplication *)__unused application didReceiveRemoteNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler
{
    ...
    NSUInteger messageType = 0;
    
    if (userInfo[@"jump_type"]) {
        messageType = ((NSString *)userInfo[@"jump_type"]).integerValue;
    }
    ...
    if ([application applicationState] != UIApplicationStateActive)
    {
        // 价格提醒
        if (messageType == 1) {
            [NSNotificationCenter.defaultCenter postNotificationName:@"PriceAlertArrive" object:nil];
            AlertHistoryViewController *vc = [[AlertHistoryViewController alloc] init];
            vc.hidesBottomBarWhenPushed = true;
            UINavigationController *navi = self.tabBarController.viewControllers[self.tabBarController.selectedIndex];
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(UNNotificationPresentationOptions))completionHandler  API_AVAILABLE(ios(10.0)) API_AVAILABLE(ios(10.0)){
    NSUInteger messageType = 0;
    if (notification.request.content.userInfo[@"jump_type"]) {
        messageType = ((NSString *)notification.request.content.userInfo[@"jump_type"]).integerValue;
    }
    if (messageType == 1) {
        [NSNotificationCenter.defaultCenter postNotificationName:@"PriceAlertArrive" object:nil];
    }
    NSString *alertType = [[NSUserDefaults standardUserDefaults] objectForKey:@"SettingAlertSettingTypeKey"];
    if (alertType == nil || [alertType isEqual: @"SoundAndVibration"]) {//}[OCLanguage SWLocalizedString:@"s_and_v"]) {
        AudioServicesPlaySystemSound(1007);
    } else if ([alertType isEqual: @"Sound"]) {//[OCLanguage SWLocalizedString:@"sound"]) {
        NSError* error;
    ...
    } else if ([alertType isEqual: @"Vibration"]) {//[OCLanguage SWLocalizedString:@"vibration"]) {
...
- (void)networkDidReceiveMessage:(NSNotification *)notification {
    ...
    [SwiftNotificationManager.shared processNewNotificationWithUserInfo:userInfo];
}
