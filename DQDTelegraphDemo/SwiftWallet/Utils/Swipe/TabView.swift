...
open class TabView: UIScrollView {
    ...
    fileprivate func setupContainerView(dataSource: TabViewDataSource) {
        ...
        containerView.spacing = options.spacing
    ...
    fileprivate func setupTabItemViews(dataSource: TabViewDataSource) {
        ...
        for index in 0..<itemCount {
            ...
            if let title = dataSource.tabView(self, titleForItemAt: index) {
                ...
                if index == 0 {
                    tabItemView.titleLabel.font = options.itemView.selectedFont
                } else {
                ...
                tabItemView.backgroundColor = options.itemView.backgroundColor
            ...
            xPosition += tabItemView.frame.size.width + containerView.spacing
        }

        layout(containerView: containerView, containerWidth: xPosition - containerView.spacing)
        addTabItemGestures()
    ...
    private func layout(containerView: UIView, containerWidth: CGFloat) {
        ...
        case .underline:
            heightConstraint = containerView.heightAnchor.constraint(equalToConstant:
                options.height
//                    - options.additionView.underline.height - options.additionView.padding.bottom
            )
        case .circle, .none:
    ...
    private func updateSelectedItem(by newIndex: Int) {
        for (i, itemView) in itemViews.enumerated() {
            ...
            if itemView.isSelected {
                itemView.titleLabel.font = itemView.selectedFont
            } else {
                itemView.titleLabel.font = itemView.font
            }
        }
    }
}
...
extension TabView {
    ...
    fileprivate func setupAdditionView() {
        ...
        case .circle:
            let itemView = itemViews[currentIndex]
            let height = ((options.additionView.circle.height != nil) ? options.additionView.circle.height! : itemView.bounds.height) - options.additionView.padding.vertical
