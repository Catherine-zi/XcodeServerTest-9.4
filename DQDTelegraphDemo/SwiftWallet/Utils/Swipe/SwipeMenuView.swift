...
open class SwipeMenuView: UIView {
    ...
    open var reuseVCClass: AnyClass?
    ...
    private var reuseVCArray: [UIViewController] = []
    fileprivate var allVCArray: [UIViewController?] = []
    fileprivate var formerReuseIndexArray: [Int] = [0, 0, 0]
    ...
    open private(set) var currentIndex: Int = 0 {
        didSet {
            ...
            contentScrollView?.prepareMorePages(currentIndex: currentIndex)
            for index in self.formerReuseIndexArray {
                if index > currentIndex + 1 || index < currentIndex - 1 {
                    if index < self.allVCArray.count {
                        self.allVCArray[index] = nil
                    }
                }
            }
            let start = currentIndex - 1 >= 0 ? currentIndex - 1 : 0
            let end = currentIndex + 1 < self.allVCArray.count ? currentIndex + 1 : currentIndex
            self.formerReuseIndexArray = [start, currentIndex, end]
        }
    }
    ...
    public func dequeueReuseVC(index: Int) -> UIViewController? {
        for vc in self.reuseVCArray {
            if !self.allVCArray.contains(vc) {
                return vc
            }
        }
        if let vcClass = self.reuseVCClass as? UIViewController.Type {
            let vc = vcClass.init()
            return vc
        }
        return nil
    ...
    private func setup(default defaultIndex: Int = 0) {
        ...
        if self.allVCArray.count != self.pageCount {
            self.allVCArray = Array.init(repeating: nil, count: self.pageCount)
        }
    ...
    private func layout(tabView: TabView) {
        ...
        if self.leftView == nil,
            ...
            tabView.snp.makeConstraints { (make) in
                ...
                make.height.equalTo(options.tabView.height)
            }
            ...
            self.rightView!.snp.makeConstraints { (make) in
                ...
                make.width.equalTo(self.rightView!.bounds.width)
                make.height.equalTo(tabView.snp.height)
            }
            ...
            self.leftView!.snp.makeConstraints { (make) in
                make.top.left.equalTo(0)
                make.width.equalTo(self.leftView!.bounds.width)
                make.height.equalTo(tabView.snp.height)
            }
        } else {
            if self.leftView?.superview != self {
...
//            ])
    ...
    private func reset() {
        ...
        self.formerReuseIndexArray[0] = (currentIndex - 1) > 0 ? currentIndex - 1 : 0
        self.formerReuseIndexArray[1] = currentIndex
        self.formerReuseIndexArray[2] = (currentIndex + 1) < self.pageCount ? currentIndex + 1 : currentIndex
...
extension SwipeMenuView: ContentScrollViewDataSource {
    ...
    public func contentScrollView(_ contentScrollView: ContentScrollView, viewForPageAt index: Int) -> UIView? {
        if index < self.allVCArray.count {
            if self.allVCArray[index] != nil {
                return self.allVCArray[index]?.view
            }
        }
        let vc = dataSource?.swipeMenuView(self, viewControllerForPageAt: index)
        if index < self.allVCArray.count {
            self.allVCArray[index] = vc
        }
        if let vc = vc {
            if !self.reuseVCArray.contains(vc) {
                self.reuseVCArray.append(vc)
            }
        }
        return vc?.view
    }
