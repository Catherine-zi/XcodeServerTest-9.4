...
- (instancetype)initWithMessage:(TGBridgeMessage *)message type:(TGNeoMessageType)type users:(NSDictionary *)users context:(TGBridgeContext *)context
{
    ...
    if (self != nil)
    {
        ...
        _nameModel = [[TGNeoLabelViewModel alloc] initWithText:[contactAttachment displayName] font:[UIFont systemFontOfSize:12 weight:UIFontWeightMedium] color:[self normalColorForMessage:message] attributes:nil];
        _nameModel.multiline = false;
        [self addSubmodel:_nameModel];
        
        _phoneModel = [[TGNeoLabelViewModel alloc] initWithText:[contactAttachment prettyPhoneNumber] font:[UIFont systemFontOfSize:12] color:[self subtitleColorForMessage:message] attributes:nil];
...
- (CGSize)layoutWithContainerSize:(CGSize)containerSize
{
    ...
    if (_userId != 0)
    {
        ...
        {
            avatarDictionary = @{ TGNeoMessageAvatarUrl: _avatarUrl };
        }
        else
        {
            avatarDictionary = @{ TGNeoMessageAvatarColor: [TGNewColor colorForUserId:_userId myUserId:_ownUserId], TGNeoMessageAvatarInitials: initials };
        }
    }
    else
    {
