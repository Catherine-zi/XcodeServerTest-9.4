...
- (void)updateSelectionInterface
{
    ...
    if (count != 0)
    {
        if (_inviteContainer == nil)
        {
            ...
            inviteLabel.text = TGLocalized(@"Contacts.InviteTobitPub");
...
- (void)channelsItemSelected
{
	...
	//qq--add
	[[SPUserEventsManager sharedManager] addCountForEvent:SWUEC_Click_NewChannel_NewMessage_Page];
...
- (void)searchShouldBeginEidt{
	if ([self isKindOfClass:[TGSelectContactController class]]) {
		[[SPUserEventsManager sharedManager] addCountForEvent:SWUEC_Click_Search_NewMessage_Page];
	}
}
