...
- (void)phonePressed:(id)item
{
    for (TGUserInfoPhoneCollectionItem *phoneItem in self.phonesSection.items)
    {
        if (item == phoneItem)
        {
            ...
            if (_supportsCalls)
            {
                TGMenuSheetButtonItemView *telegramItem = [[TGMenuSheetButtonItemView alloc] initWithTitle:TGLocalized(@"UserInfo.bitPubCall") type:TGMenuSheetButtonTypeDefault action:^
                {
                    __strong TGTelegraphUserInfoController *strongSelf = weakSelf;
                    if (strongSelf != nil)
