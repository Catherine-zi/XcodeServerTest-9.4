...
//        }
		[_detailNavigationController setViewControllers:@[_dialogListController, contentController] animated:true];
		[TGAppDelegateInstance.tabBarController.tabBar setHidden:YES];
        if (addDetail) {
            [self addDetailController];
        }
...
- (void)updateSizeClass {
    if (_currentSizeClass == UIUserInterfaceSizeClassCompact) {
        ...
        if (_masterNavigationController.viewControllers.count != 0) {
			...
			[viewControllers addObject:_dialogListController];
...
- (void)localizationUpdated {
	//qq--change
	[_dialogListController localizationUpdated];
