...
- (void)updateSendButtonVisibility:(bool)animated
{
    if ([self currentlyDisplayingContextResult]) {
        ...
        _sendButton.alpha = 1.0f;  //Avazu 20180610 0.0f --> 1.0f
...
//            disappearingButtons = TGIsPad() ? @[_sendButton] : @[_sendButton, _stickersArrowButton];
            ...
            disappearingButtons = @[_stickersArrowButton];
        }
        else
        {
            appearingButton = _sendButton;
...
- (void)sendButtonPressed
{
    ...
    //Avazu 20180610 如果数据为空，则不发送
    if (text == nil || [text isKindOfClass:[NSNull class]] || ([text isKindOfClass:[NSString class]] && text.length == 0)) {
        return;
    }
