...
- (void)layoutSubviews
{
    ...
    if (_secondaryActionButton.hidden) {
        _actionButton.frame = CGRectInset(self.bounds, 40.0f + self.safeAreaInset.left, 0.0f);
    } else if (_actionButton.hidden) {
        _secondaryActionButton.frame = CGRectInset(self.bounds, 40.0f + self.safeAreaInset.left, 0.0f);
    } else {
        CGRect bounds = self.bounds;
        CGFloat width = bounds.size.width - self.safeAreaInset.left - self.safeAreaInset.right;
        _actionButton.frame = CGRectMake(40.0f + self.safeAreaInset.left, 0.0, CGFloor((width - 80.0f) / 2.0f), bounds.size.height);
        _secondaryActionButton.frame = CGRectMake(CGRectGetMaxX(_actionButton.frame), 0.0, CGFloor((width - 80.0f) / 2.0f), bounds.size.height);
    }
    
    CGRect closeButtonFrame = _closeButton.frame;
    closeButtonFrame.origin = CGPointMake(self.frame.size.width - 4.0f - TGScreenPixel - closeButtonFrame.size.width - self.safeAreaInset.right, TGScreenPixel);
