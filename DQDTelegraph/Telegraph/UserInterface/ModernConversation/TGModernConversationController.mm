    ...
    if (mediaMessageItem != nil && index >= 0)
    {
        ...
        if (previewMode)
        {
            ...
            else
            {
                ...
                modernGallery.preferredContentSize = TGFitSizeNew(dimensions, screenSize);
            }
        }
        ...
        if (messageItem != nil && messageItem->_message.mid == messageId)
        {
            ...
            for (id attachment in messageItem->_message.mediaAttachments)
            {
                ...
                else if ([attachment isKindOfClass:[TGActionMediaAttachment class]])
                {
                    ...
                    {
                        NSString *path = [[DQDTelegraphManager dqd_documentsPath] stringByAppendingPathComponent:@"calls"];
...
- (void)showActionsMenuForContact:(TGUser *)contact isContact:(bool)isContact
{
    ...
    if (contact.uid > 0) {
        ...
        [actions addObject:[[TGActionSheetAction alloc] initWithTitle:TGLocalized(@"UserInfo.bitPubCall") action:@"telegramCall"]];
    }
...
{
    /* Avazu 20180609 根据产品要求，仅保留拍照和相册
    ...
    NSMutableArray *actions = [[NSMutableArray alloc] initWithArray:@
                               [
                                [[TGActionSheetAction alloc] initWithTitle:TGLocalized(@"AttachmentMenu.PhotoOrVideo") action:@"photoOrVideo"], [[TGActionSheetAction alloc] initWithTitle:TGLocalized(@"Common.Cancel") action:@"cancel" type:TGActionSheetActionTypeCancel]]];
