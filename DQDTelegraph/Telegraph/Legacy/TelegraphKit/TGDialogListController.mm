...
//qq--add
@property (nonatomic, strong) UIButton *recGroupsBtn;
@end
...
//                [strongSelf openProxySettings];
	...
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
		...
		if (screenSize.height == 812.0f){
			tableFrame = CGRectMake(tableFrame.origin.x, tableFrame.origin.y, tableFrame.size.width, tableFrame.size.height - 49 - 34);
		}else{
			tableFrame = CGRectMake(tableFrame.origin.x, tableFrame.origin.y, tableFrame.size.width, tableFrame.size.height - 49);
		}
	...
	//qq--add
	_recGroupsBtn = [[UIButton alloc] init];
	[_recGroupsBtn setImage:[UIImage imageNamed:@"iconcontact"] forState:UIControlStateNormal];
	...
	_recGroupsBtn.frame = CGRectMake(self.view.bounds.size.width - 81, self.view.bounds.size.height - offset, 76, 76);
	[_recGroupsBtn addTarget:self action:@selector(goToRecGroupsPage) forControlEvents:UIControlEventTouchUpInside];
	[self.view addSubview:_recGroupsBtn];
	[self.view bringSubviewToFront:_recGroupsBtn];
	
	[[SPUserEventsManager sharedManager] addCountForEvent:SWUEC_Show_edit_ChatPage];
	//qq--end
}
...
- (void)viewDidAppear:(BOOL)animated
{
	...
	//qq--对话页展示时显示TabBar
	if (self.navigationController.viewControllers.count == 1 && [self.navigationController.viewControllers.firstObject isKindOfClass:[self class]]) {
		[TGAppDelegateInstance.tabBarController.tabBar setHidden:false];
	}
}
...
- (void)composeMessageButtonPressed:(id)__unused sender
{
	...
	//qq--add
	[[SPUserEventsManager sharedManager] addCountForEvent:SWUEC_Click_edit_ChatPage];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _tableView)
    {
        ...
        if (cell.selectionStyle != UITableViewCellSelectionStyleNone)
        {
            ...
            {
				//qq--add 新增过滤系统
				NSString * fileterStr = [NSString stringWithFormat:@"%@", conversation.dialogListData[@"title"]].uppercaseString;
				if (![self filterInsensitiveContent:fileterStr]) {
    ...
    else
    {
        ...
        {
			//qq--add （添加一个过滤条件，把原有的代码放在过滤的外边）
			TGConversation *conversation = (TGConversation *)result;
			if ([self filterInsensitiveContent:[NSString stringWithFormat:@"%@",conversation.dialogListData[@"title"]].uppercaseString]) {
        ...
        {
			TGUser *user = (TGUser *)result;
			if (![self filterInsensitiveContent:[NSString stringWithFormat:@"%@",user.firstName].uppercaseString] && ![self filterInsensitiveContent:user.userName]) {
				[_searchDisposable setDisposable:nil];
...
- (BOOL)filterInsensitiveContent:(NSString *)title {
	
	if ([title containsString:@"YOUTUBE"] || [title containsString:@"URL UPLOADER"]) {
		//弹窗
		UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:[OCLanguage SWLocalizedString:@"TelegramFilterTitle"] message:[OCLanguage SWLocalizedString:@"TelegramFilterContent"] preferredStyle:UIAlertControllerStyleAlert];
		UIAlertAction *ok = [UIAlertAction actionWithTitle:[OCLanguage SWLocalizedString:@"TelegramFilterOk"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
//qq--and
- (void)addSearchContext:(NSString *)searchQuery withBlock:(SearchSuccessBlock)block{
	if (_searchDisposable == nil)
		_searchDisposable = [[SMetaDisposable alloc] init];
	__weak TGDialogListController *weakSelf = self;
	
	id<SDisposable> disposable = [[[TGGlobalMessageSearchSignals search:searchQuery includeMessages:!_dialogListCompanion.forwardMode itemMapping:^id(id item)
	   {
		   NSLog(@"%@",item);
		   weakSelf.item = item;
		   return nil;
	...
	[_searchDisposable setDisposable:disposable];
}
//qq--注释，搜索框输入一个链接后
...
//qq--add
- (void)searchShouldBeginEidt{
	[[SPUserEventsManager sharedManager] addCountForEvent:SWUEC_Click_Search_ChatPage];
}
