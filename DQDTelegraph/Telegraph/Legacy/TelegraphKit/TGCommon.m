...
static NSString *currentNativeExtensionLocalizationPath() {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        path = [[DQDTelegraphManager dqd_documentsPath] stringByAppendingPathComponent:@"localization"];
    });
    return path;
}
...
static NSString *legacyCustomLocalizationBundlePath() {
    return [[DQDTelegraphManager dqd_documentsPath] stringByAppendingPathComponent:@"CustomLocalization.bundle"];
}
...
//    return [effectiveLocalization() get:s];
	return [OCLanguage SWLocalizedString:s];
}
