        ...
        if (config != nil) {
            ...
            return [[[TGTelegramNetworking instance] requestSignal:getDhConfig] mapToSignal:^SSignal *(TLmessages_DhConfig *config) {
                if ([config isKindOfClass:[TLmessages_DhConfig$messages_dhConfig class]])
                {
                    ...
                    if (!MTCheckMod(concreteConfig.p, concreteConfig.g, [MTFileBasedKeychain keychainWithName:@"legacyPrimes" documentsPath:[DQDTelegraphManager dqd_documentsPath]]))
                    {
                        return [SSignal fail:nil];
                    }
                    
                    if (!MTCheckIsSafePrime(concreteConfig.p, [MTFileBasedKeychain keychainWithName:@"legacyPrimes" documentsPath:[DQDTelegraphManager dqd_documentsPath]]))
                    {
                        return [SSignal fail:nil];
                    }
...
+ (SSignal *)reportCallRatingWithCallId:(int64_t)callId accessHash:(int64_t)accessHash rating:(int32_t)rating comment:(NSString *)comment includeLogs:(bool)includeLogs
{
    ...
    if (includeLogs)
    {
        NSString *logsPath = [[DQDTelegraphManager dqd_documentsPath] stringByAppendingPathComponent:@"calls"];
