...
//Avazu 20180607 重命名为TGRecentSearchDefaultsKeyNew
NSString *const TGRecentSearchDefaultsKeyNew = @"Telegram_recentSearch_peers";
//Avazu 20180607 重命名
const NSInteger TGRecentSearchLimitNew = 20;
...
{
    [[self userDefaults] removeObjectForKey:TGRecentSearchDefaultsKeyNew];
...
+ (void)addRecentPeerResult:(TGPeerId)peerId
{
    ...
    NSMutableArray *items = [[NSMutableArray alloc] initWithArray:[[self userDefaults] objectForKey:TGRecentSearchDefaultsKeyNew]];
    [items removeObject:@(appPeerId)];
    [items insertObject:@(appPeerId) atIndex:0];
    if (items.count > TGRecentSearchLimitNew)
        [items removeObjectsInRange:NSMakeRange(TGRecentSearchLimitNew, items.count - TGRecentSearchLimitNew)];
    [[self userDefaults] setObject:items forKey:TGRecentSearchDefaultsKeyNew];
    [[self userDefaults] synchronize];
}
...
+ (SSignal *)recentPeerResultsWithContext:(TGShareContext *)context cachedChats:(NSArray *)cachedChats
{
    ...
    NSArray *recentChatsIds = [[self userDefaults] objectForKey:TGRecentSearchDefaultsKeyNew];
