...
//Avazu 20180608 Use TGScaleImage
...
@end
//Avazu 20180607 使用TGGeometry中的即可
CGSize TGFitSizeNew(CGSize size, CGSize maxSize)
{
    if (size.width < 1)
        size.width = 1;
    if (size.height < 1)
...
//Avazu 20180607 使用TGGeometry中的即可
CGSize TGFillSizeNew(CGSize size, CGSize maxSize)
{
    if (size.width < 1)
        size.width = 1;
    if (size.height < 1)
...
bool TGIsRetina()
{
    ...
    if (!initialized)
    {
        //320 * 480 为 1  640 * 960 为 2 1242 x 2208 为 3
